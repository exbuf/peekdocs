...
✅ TRANSLATED — six languages (English, Español, Français, Deutsch,
日本語, 简体中文), 102 keys × 6 = 612 entries:
...
_STRINGS = {
    ...
    "zh-CN": {
        ...
        "delete_on_close_tooltip": "关闭 peekdocs 时，自动删除会话期间搜索的每个文件夹中的所有搜索结果文件（peekdocs_standard_results.*、peekdocs_regex_results.*、peekdocs_suite_results.*）和搜索索引（.peekdocs.db）。包括索引是因为它包含从每个已索引文件提取的文本。您可以随时勾选或取消勾选 — 仅在关闭应用程序时才有意义。已保存的报告（peekdocs_report_*）、累积报告（peekdocs_accumulated_*）、已保存的搜索、设置和书签永不删除。对于会话中的即时清理，请使用工具 → 清除文件 → 擦除会话。",
