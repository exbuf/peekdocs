"""PeekDocs GUI — HelpPanelsMixin.

Extracted from ``_mixin_tools.py`` in the mixin-tools-split refactor.
Owns the 8 orphan "?"-help popups that don't belong to a specific
feature mixin. Each opens a themed scrollable text window with a
table-of-contents and prose sections explaining a specific area of
peekdocs's UI or behavior.

Method surface:

  _show_search_options_help    "?" next to Search Bar — the general
                               "how the search bar works" primer.
  _show_search_help            "?" for the main-page search primer
                               (three modes: Standard/Suites/Regex).
  _show_advanced_help          "?" in Advanced Search Options — deep
                               dive on all the tuning knobs.
  _show_save_load_help         "?" for Save Search / Load Search
                               popup — file-scope, precedence rules.
  _show_matched_files_help     "?" in Matched Files popup — heatmap,
                               View Text, double-click behavior.
  _show_excluded_files_help    "?" in Excluded Files popup — skip
                               reasons taxonomy.
  _show_index_help             "?" in Manage Indexes — how the FTS5
                               index works, cost/benefit.
  _show_search_modes_compare   Side-by-side comparison of Standard /
                               Suites / Regex — when to use each.

External callers (all resolve via PeekDocsApp's MRO after extraction):

  _mixin_build.py:462   _show_save_load_help
  _mixin_build.py:534   _show_search_help
  _mixin_build.py:666   _show_advanced_help
  _mixin_build.py:1674  _show_index_help
  _mixin_data.py:1187   _show_matched_files_help (via lambda)
  _mixin_data.py:1404   _show_excluded_files_help (via lambda)
"""
