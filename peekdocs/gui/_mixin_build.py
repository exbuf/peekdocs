...
class BuildMixin:
    def _build_getting_started_tab(self):
        ...
        _step(3, "Use Advanced Search Options below to configure search parameters", "Step 3 on the main page is a pointer to the inline Advanced Search Options panel — click the ▶ Advanced Search Options header in the left pane to expand it. Inside you'll find: AND/OR mode, Recursive, Whole Word, Regex, Fuzzy, Wildcard, OCR, Inverse, Expression mode, Use Index, file-type filters, exclude terms, proximity, context lines, CPU cores, max matches, max file size, range filters, specific-file list, output directory, the optional output formats DOCX / CSV / JSON / PDF / HTML, timestamp filenames, Delete on Close, Clear history on close, Restrict permissions, and Notify on Search Complete.\n\nTXT is always written (the in-pane Results Preview and Matched Files popup both read from it). DOCX / CSV / JSON / PDF / HTML all default OFF — check the ones you want (CLI: `-o docx,csv,json,pdf,html`, any combination). The Open Report row in the left pane lights its buttons green for each format the most recent search produced — click any green button to open that format. Reports stay on your computer; peekdocs itself never uploads anything, and Open Report hands the file to whatever app you've set as your OS default for that file type.\n\nWhich output formats apply where: the CSV / JSON / PDF / HTML checkboxes apply to Standard Search only. Search Suites and Regex Search each have their own picker inside their popup (TXT always; DOCX / HTML / CSV / JSON / PDF opt-in per checkbox). After any Suite or Regex run, the Open Report buttons re-point to whatever that run produced, so a button colored red just means that format wasn't generated by the most recent search — not that anything is broken.")
        _step(4, "Click Run Standard Search (or Search Suites / Regex Search)", "Three Run buttons share the Step 4 row:\n\n  • 🔍 Run Standard Search (blue, large) — runs your typed terms against the current Advanced Search Options settings (AND/OR, Recursive, Whole Word, Use Index, regex, fuzzy, file-type filters, output formats, and the rest). This is the everyday button.\n  • Search Suites (green, square) — opens a popup of named suites (groups of saved searches) that run together as a single combined report. Each saved search inside a suite carries its own settings, so the Advanced Search Options on the main page do NOT apply to suite runs. The suite popup has its own DOCX / HTML / CSV / JSON / PDF picker (TXT always; the rest opt-in per checkbox).\n  • Regex Search (orange, square) — opens the multi-pattern regex collections popup. Each pattern in a collection is independent and runs as its own regex. Like Search Suites, the Advanced Search Options on the main page do NOT apply — the popup has its own folder picker, Recursive checkbox, and 'Also write:' output-format checkboxes (TXT always; DOCX / HTML / CSV / JSON / PDF opt-in). Use this when you need several regex patterns at once; the Standard Search Regex checkbox is for a single pattern only.\n\nAfter the search, results appear with matches highlighted in yellow. The right pane shows a results headline (files searched · matches · elapsed time), Matched Files / Excluded Files count buttons, a Chart button (top files by match count), and the matches themselves. On the left pane, click any green button in the Open Report row to open the highlighted DOCX, TXT, CSV, JSON, PDF, or HTML report (each opens in whatever app you've set as your OS default for that file type).")
        ...
        features = [
            ("\U0001f9ea  Search Wizard", "Click the blue Search Wizard button on the main page (between Run Standard Search and Search Suites). Pick a search type (phone, email, dollar range, date, etc.) and the wizard fills in the search bar + Advanced Search Options for you.", "#8B5CF6"),
            ("\U0001f50e  Save and Reload Searches", "▶ Save the current search by name; ▶ Reload to recall the full configuration later.", "#2196F3"),
            ("\U0001f4c4  Highlighted Reports", "Opt into a Word report (DOCX checkbox in Advanced Search Options, or `-o docx` on the CLI) and get matches highlighted in yellow. HTML, CSV, JSON, and PDF are opt-in the same way.", "#E65100"),
    ...
    def _show_welcome(self):
        ...
        b("Results are saved as a .txt report by default; opt into a")
        b("highlighted Word (DOCX) report via the DOCX checkbox in")
        ...
        b("CSV / JSON / PDF / HTML all default OFF — check the ones")
        b("you want. Search Suites and Regex Search each have their")
        b("own format picker inside their popup (TXT always; DOCX /")
        b("HTML / CSV / JSON / PDF opt-in per checkbox). After a Suite")
        b("or Regex run, the Open Report buttons (on the left pane")
        b("below the status row) re-point to whatever that run produced")
        b("— a red button just means that format wasn't generated by")
        b("the most recent search.")
        blank()
        ...
        b("  where each pattern is independent. Like Search Suites,")
        b("  Advanced Search Options on the main page do NOT apply;")
        b("  the popup has its own folder, Recursive checkbox, and")
        b("  output-format checkboxes (TXT always; DOCX / HTML /")
        b("  CSV / JSON / PDF opt-in).")
        blank()
    ...
    def _build_advanced_panel(self):
        ...
        # column 0 and all five checkboxes share one line. All five
        # default OFF (DOCX joined the opt-in formats in 1.2.6); TXT
        # is the only always-written format. Tooltip on each format
        # checkbox documents the same scope note.
        _output_scope_note = (
            "Applies to Standard Search only (the green Run button). "
            "Search Suites and Regex Search each have their own "
            "DOCX / HTML / CSV / JSON / PDF picker inside their popup; "
            "the checkboxes here have no effect on those workflows."
        )
        ...
        Tooltip(cb_docx,
                "Also write the highlighted Word report (.docx). Default "
                "OFF (DOCX joined the opt-in formats in 1.2.6). Check to "
                "produce a .docx alongside the always-written .txt — "
                "yellow-highlighted matches, file paths as section "
                "headings, line numbers, surrounding context preserved. "
                + _output_scope_note,
                anchor="above")
        ...
        Tooltip(self.max_matches_entry, "Maximum matches included in reports. Default 5000. Set to 0 for no limit. The cap protects you from minutes-long DOCX renders on huge result sets (python-docx is slow at inserting tens of thousands of highlighted runs) and from massive report files. Raise it (or 0 = unlimited) when you really want every match \u2014 typical use of 0 is jq-pipeline scenarios where the report is TXT or JSON only. See \u2753 help for the full discussion plus the counterintuitive interaction with Max File Size.")
    ...
    def _build_progress_area(self):
        ...
        # Track the pane width so the rest-of-headline wraps cleanly
        # within the space left over after the yellow highlight chip.
