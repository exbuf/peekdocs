...
class BuildMixin:
    def _build_getting_started_tab(self):
        ...
        _step(3, "Output formats", "TXT and DOCX are always generated. Optional formats (CSV, JSON, PDF, HTML) can be enabled in Advanced Search Options under 'Also output report as'. After a search runs, the buttons in this row light up — click any colored button to open that format. Buttons stay red if the format wasn't generated by the most recent search. Reports stay on your computer — peekdocs avoids opening them in Google Docs, Apple Pages, or any cloud-based application that may upload your data. Check Delete on Close to automatically remove result files when you close the app.\n\nWhich output formats apply where: the CSV / JSON / PDF / HTML checkboxes apply to Standard Search only (the blue Run button below). Search Suites have their own HTML / CSV / JSON / PDF picker inside the Search Suites popup. Regex Search (and Run Multiple Collections) always writes just TXT and DOCX. After any Suite or Regex run, the Step 3 report buttons re-point to whatever that run produced, so a button colored red just means that format wasn't generated by the most recent search — not that anything is broken.")
        _step(4, "Click Run Standard Search", "The blue options bar just above (AND/OR, Recursive, Whole Word, Use Index) applies to this blue Run Standard Search button — Search Suites (green) and Regex Search (orange) have their own settings and open management popups. peekdocs scans every supported file and shows results with matches highlighted in yellow. Scan matches in the Results Preview pane that appears below, or click DOCX / TXT in the Step 3 row above for a highlighted report (DOCX opens in Microsoft Word or LibreOffice; HTML in your browser if enabled).")
    ...
    def _build_progress_area(self):
        ...
        # App Size + Language dropdowns live on the right side of the
        # main page header row (created in _build_search_row). Pack
        # order is right→left: lang_picker, lang_label, app_size_menu,
        # app_size_lbl. Preview Size moves to the Results Preview row
        # below — see preview_header_mid further down.
        #
        ...
        self._lang_label = ctk.CTkLabel(self._page_header_row, text=_t_lang("language_picker_label"), font=ctk.CTkFont(size=11))
        ...
        self._app_size_lbl = ctk.CTkLabel(self._page_header_row, text=__import__("peekdocs.i18n", fromlist=["t"]).t("app_size_label"), font=ctk.CTkFont(size=11))
