...
class BuildMixin:
    ...
    def _show_welcome(self):
        ...
        e("budget revenue")
        blank()
        st("Step 3: Output formats")
        b("TXT and DOCX are always generated. Optional formats")
        b("(CSV, JSON, PDF, HTML) can be enabled in Advanced Search")
        b("Options under 'Also output report as'. After a search runs,")
        b("the buttons in this row light up — click any colored button")
        b("to open that format. Buttons stay red if the format wasn't")
        b("generated by the most recent search.")
        blank()
        b("Output formats by mode. The CSV / JSON / PDF / HTML")
        b("checkboxes apply to Standard Search only. Search Suites")
        b("have their own format picker inside the Suites popup,")
        b("and Regex Search always writes just TXT and DOCX. After")
        b("a Suite or Regex run, the Step 3 buttons re-point to")
        b("whatever that run produced — a red button just means")
        b("that format was not generated by the most recent search.")
        blank()
        st("Step 4: Click Run Standard Search")
        ...
        b("(orange) have their own settings.")
        blank()
        ...
        blank()
        b("Click the DOCX button in the Step 3 row above to open")
        b("the Word report with your matches highlighted in yellow.")
        blank()
    ...
    def _build_advanced_panel(self):
        ...
        Tooltip(self._adv_save_btn, "Save all current options as permanent defaults to ~/.peekdocsrc. This includes all settings on this Advanced Search Options screen and on the main screen (search terms, folder, recursive, AND/OR mode, whole word, etc.). These become the defaults every time you launch the app", anchor="above")
