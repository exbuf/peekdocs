...
Results are saved to `peekdocs_standard_results.txt` in the current directory. TXT is always written (the GUI's Results Preview parses it). DOCX, CSV, JSON, PDF, and HTML are opt-in via the Advanced Search Options checkboxes (GUI) or `-o docx,csv,json,pdf,html` on the CLI (any combination); when DOCX is opted in, `peekdocs_standard_results.docx` is written alongside the .txt with yellow-highlighted matches. Each report includes the date and time, the command used, search terms, number of hits, search time, number of files searched, total file size, and a file type tally. Each match shows the document name, directory path, line number, and the matched text with search terms highlighted — `**bold**` markers in the `.txt` file and yellow highlighting in the `.docx` / `.html` / `.pdf` files. Note that these result files are overwritten each time you run a new search. Use the `-s` flag to archive them or the `-sa` flag to accumulate results across searches. Archived files are saved as `peekdocs_report_my_report.txt` and accumulated files as `peekdocs_accumulated_my_report.txt` — the prefix is added automatically so they are never re-searched in future searches.
...
Two different things can cause this, depending on whether you already have an index built.

*First search in a folder that has never been indexed:* peekdocs builds the optional search index by reading every file once and storing extracted text in `.peekdocs.db`. This takes anywhere from a few seconds (small folders) to a few minutes (thousands of files, large PDFs, or scanned documents). Every search after that uses the index and runs in milliseconds. The index keeps itself current automatically; you don't need to rebuild it manually. If you'd rather not use the index, add `--no-index` to your CLI command or uncheck **Use Index** in the GUI — searches will then read files directly each time.

*First search of a session, even with the index already built:* you'll often see something like 2.5 seconds on the first search and 0.5 seconds on the next, with no rebuild in between. That's cold-cache overhead, not a rebuild. Three layers contribute, in decreasing order of cost: (1) the OS hasn't recently read your `.peekdocs.db` (often hundreds of MB) so it reads from SSD on the first open and from memory on subsequent ones; (2) every fresh `peekdocs` invocation re-pays Python interpreter startup + library imports — typically a few hundred ms before the search timer even starts; (3) `refresh_index` does a quick `os.stat()` pass on every file in the folder to spot changes, and the first pass hits disk while subsequent passes hit the directory cache. The 0.5 s number is your steady-state performance; the 2.5 s first hit is the price of being absent from the OS cache. If you want a fast first interactive search, the only practical lever is to keep the cache warm — e.g., schedule `peekdocs --index-refresh` from a launchd / systemd / Task Scheduler job at login so your first interactive search later in the session hits an already-warm cache.

For scheduled jobs that want absolutely silent output, including any messages during the index-build phase, use standard shell redirection: `peekdocs TODO -r -qq 2>/dev/null` — this works alongside any quiet flag and discards every stderr message peekdocs (or any other tool) might emit. See [Search Index (Optional)](USER_GUIDE.md#search-index-optional) in the User Guide for full details, or run `peekdocs --check` to see your current index size.
...
- **DOCX wasn't checked for this run:** DOCX is opt-in as of 1.2.6 — it defaults OFF in Advanced Search Options. If you didn't check the **DOCX** box (GUI) or pass `-o docx` (CLI) for your most recent run, no `peekdocs_standard_results.docx` exists to open. Check the DOCX box, re-run the search, then click the DOCX button again. (When a format wasn't generated by the most recent search, its Open Report button shows red as a hint.)
- **No word processor installed:** You need a program that can open `.docx` files. Microsoft Word works, but it's not required. [LibreOffice](https://www.libreoffice.org/download/download-libreoffice/) (free, cross-platform) is a common choice. After installing one, try clicking the DOCX button again. You can also enable HTML output in Advanced Search Options — every computer has a browser.
