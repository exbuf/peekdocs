...
**Mixing industries:** You're not limited to one template. Run the Compliance Wizard multiple times with different industries — all saved searches go into the same folder's collection. Then open **Manage Suites** and build a custom suite that combines checks from any industry. For example, a medical billing office might combine HIPAA patient checks with financial services checks and HR employee checks into one comprehensive suite.

**Importing custom templates:** Click **Import Template** in the Manage Suites panel to load saved searches and suites from an external `.json` template file. The imported items are merged into your existing collection without overwriting non-conflicting items. Use this to receive custom templates from a consultant or to share suites between teams and folders.

**Exporting suites:** Select a suite and click **Export Suite** to save it as a `.json` file containing the suite and all its saved searches. Send the file to a colleague or client — they click **Import Template** to load it. This is the recommended way to distribute compliance suites across teams or machines.

**Building your own templates:** The 9 industry templates are starting points, not a fixed set. You can create compliance suites for any industry, regulation, or internal standard — there is no limit. Build your own saved searches (using the Search Wizard, search bar, or Advanced Search Options), group them into a suite with pass/fail criteria, and export the suite as a `.json` file. The result is functionally identical to a wizard-generated template. There is no difference between a suite created by the wizard and one you build yourself.
