...
`.github/workflows/test.yml` runs the full pytest suite (~718 tests) on **Windows + macOS + Linux × Python 3.10 / 3.11 / 3.12 / 3.13 / 3.14** with `fail-fast: false`. Every internal Python code path is exercised cross-platform on every push and pull request. `build-release.yml` additionally builds the PyInstaller standalones on all three OSes on every release tag.
