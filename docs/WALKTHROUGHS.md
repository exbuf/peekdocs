...
**(d) Word report — the shareable artifact.** Every Standard Search produces `peekdocs_standard_results.docx` by default alongside the always-written .txt (uncheck the **DOCX** checkbox in Advanced Search Options, or pass `--no-docx` on the CLI, to skip the Word report) — yellow-highlighted matches, file paths as section headings, line numbers, surrounding context preserved. Hand it to a colleague who's never heard of peekdocs and they immediately understand what's in it. Opens in Microsoft Word or [LibreOffice](https://www.libreoffice.org/download/download-libreoffice/) (free) — the screenshot below is LibreOffice. peekdocs avoids opening reports in cloud-based apps like Google Docs or Apple Pages (see [For IT and Security Teams](../README.md#for-it-and-security-teams) in the README). Optional CSV, JSON, PDF, and HTML outputs are also available (checkboxes under Advanced Search Options) — these apply to Standard Search only. Search Suites have their own format picker inside the Suites popup, and Regex Search always writes just TXT and DOCX.
