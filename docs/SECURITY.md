...
- **Security disclosure.** Suspected security issues should be reported by opening a private GitHub security advisory at the project's repository. Responses come when available; no embargoed-disclosure clock is committed to. Public reporters who post details before a maintainer response remains the reporter's choice; the maintainer does not request embargo.
