...
| Where | Chart |
|---|---|
| Right pane → **Chart-File Match Count** button (next to Clear) | Top 10 files by match count (horizontal bar) |
| Right pane → **Chart-File Type Count** button (next to Chart-File Match Count) | Matches grouped by file extension, alphabetical, with per-type totals plus the count of distinct file types searched (vertical scroll when many types) |
...
> **Output formats — which checkbox applies to which workflow.** The DOCX / CSV / JSON / PDF / HTML checkboxes inside Advanced Search Options, under "Also ==>", apply to **Standard Search only** — the green Run Standard Search button on the main page. DOCX defaults to ON; uncheck it (or pass `--no-docx` on the CLI) to skip the highlighted Word report. CSV / JSON / PDF / HTML default to OFF — check the ones you want. **TXT is always written** and cannot be disabled, because the Results Preview pane, the Matched Files popup, and the match-heatmap chart all parse it. **Search Suites** have their own HTML / CSV / JSON / PDF picker inside the Search Suites popup; changes you make to Advanced Search Options have no effect on suite runs. **Regex Search** always writes just TXT and DOCX, with no per-format selection — the Advanced Search Options checkboxes don't apply. After a Suite or Regex run completes, the **Open Report:** buttons on the main page re-point to whatever the run just produced, so a button showing red just means that format was not generated by the most recent search — not that anything is broken.
...
| `-o` (output) | Additional output formats — `csv`, `json`, `pdf`, `html`, or any combination (`csv,json,pdf,html`). The `.txt` report is always written (the GUI Results Preview parses it); `.docx` is written by default but can be skipped via `--no-docx`. `-o` adds the optional formats on top of these |
| `--no-docx` | Skip writing the `.docx` (highlighted Word) report. Standard Search only — Suites and Regex collections always write `.docx`. Use when you only need the `.txt` (or `-o` formats) and want to avoid the Word render cost on every run |
...
- A `.docx` report is also generated by default; uncheck **DOCX** in Advanced Search Options (or pass `--no-docx` on the CLI) to skip it
