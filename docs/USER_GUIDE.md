...
- **Census questions — send these to peekdocs directly.** The answer needs the *whole* result set: an exact count, an exhaustive list, an aggregate. *"How many documents mention 'invoice'?"*, *"List **every** client I've billed."* Because responses are capped (`--max-results`, kept low for local models so replies fit the model's context window), the assistant only sees the first slice and can answer confidently from a partial picture. peekdocs' own counts and its CSV/JSON exports are complete and exact — use those.
