...
| **File Age Distribution** | Groups every file by modification age into six buckets — 0–6 months, 6 months–1 year, 1–3 years, 3–5 years, 5–10 years, 10+ years — and shows an ASCII histogram plus per-bucket file lists with date, size, and relative path. Useful for archives, records management, and personal document collections: surfaces stale folders that haven't been touched in years, shows what fraction of a collection is recent activity vs. long-untouched material, and helps with retention reviews. Click **Save Report** to export as a plain-text file. |
