...
The `peekdocs.api` module ships Python type hints on every public function (`search`, `list_suites`, `run_suite`, `list_regex_collections`, `run_regex_collection`) and every public dataclass (`SearchMatch`, `SearchResult`, `SuiteSearchResult`, `SuiteResult`, `PatternResult`, `CollectionResult`). Modern IDEs (PyCharm, VS Code with Pylance) provide autocomplete and inline type checking without additional stubs; `mypy` verification works out of the box against the shipped `.py` files — no separate `.pyi` stubs required. A mypy check runs in CI on every push and PR to `main` — currently covering `api.py`, `paths.py`, `reporter.py`, `cli.py`, `errors.py`, and the extracted subcommand modules under `commands/` (8 files total; the authoritative list lives in `pyproject.toml`'s `[tool.mypy]` block) — so the type contract stays honest and signature drift between the code and its documented types fails the build.
