...
- **Type-check gate.** `mypy` runs on `peekdocs/api.py`, `peekdocs/paths.py`, `peekdocs/reporter.py`, and `peekdocs/cli.py` on every push and PR to `main`. Config lives under `[tool.mypy]` in `pyproject.toml`. Guards against signature drift — for example, a callback typed `Callable[[int, int], None]` (2-arg) when the actual call site passes 3 arguments would fail the build. Coverage widens with each pass; the scanner, indexer, GUI mixins, and internal helper modules remain out of scope for now (tracked in *Known weaknesses* below).
