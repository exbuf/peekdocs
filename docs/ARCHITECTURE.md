...
| `paths.py` | Shared path + platform helpers. `resource_path()` for locating bundled files inside PyInstaller bundles or source checkouts (searches `_MEIPASS` + the macOS `.app` `Contents/Resources/` fallback). `read_bundled_text(name)` — universal reader for LICENSE / NOTICE / THIRD_PARTY_NOTICES.md that tries `resource_path` first, then falls back to `importlib.metadata.distribution("peekdocs").read_text(...)` for pip / pipx installs where the file lives in `.dist-info/licenses/` per PEP 639. `find_tesseract()` for locating the Tesseract binary with well-known-path fallback on the macOS GUI-launch stripped-PATH trap. `format_bytes()` — single source of truth for human-readable byte formatting, consumed by `reporter.fmt_size`, `cli._dry_run_report`, and the GUI file-analysis mixin. |
| `errors.py` | Public exception hierarchy. `PeekdocsError` root, `QueryError` (bad search input), `RangeError` (malformed `-R` spec), `NameNotFoundError` (missing suite/collection). Each subclass inherits from the closest stdlib exception (`ValueError`, `KeyError`) so existing consumer code that catches those types keeps working — this is a non-breaking upgrade for anyone already handling errors from `peekdocs.api`. |
...
- **Type-check gate.** `mypy` runs on the public API surface + core engine modules on every push and PR to `main` — currently `api.py`, `paths.py`, `reporter.py`, `cli.py`, `errors.py`, `scanner.py`, `indexer.py`, the six extracted subcommand modules under `commands/`, and `gui/_error_guard.py` (14 files total). Authoritative list lives in `[tool.mypy]` in `pyproject.toml`. Guards against signature drift — for example, a callback typed `Callable[[int, int], None]` (2-arg) when the actual call site passes 3 arguments would fail the build. With scanner + indexer in scope, the entire engine path from discovery through extraction, indexing, and search is now type-checked end to end; the GUI mixins (except `_error_guard`) and remaining helper modules stay out of scope for now (tracked in *Known weaknesses* below).
...
- **Silent exception swallowing in GUI mixins (partial fix in progress).** ~149 `except Exception: pass` sites across the GUI mixins mix legitimate defensive Tk guards (widget-destroyed races, focus timing) with bug swallowers. The infrastructure fix landed in v1.2.79 — `peekdocs.gui._error_guard` provides `gui_guard` (swallow + log to `peekdocs_errors.log` with call-site info) and `gui_race_guard` (swallow silently for known Tk races). A bounded set of persistence sites in `_mixin_data.py` (config save, bookmarks save, history-file clear, factory-reset rc-file remove) are converted as a pattern demonstration; the remaining sites are one-at-a-time work.
