...
- **Report file permissions.** On Unix/macOS, all report files are set to owner-only read/write (chmod 600) immediately after creation. On Windows, files inherit the user's default NTFS permissions, which are typically restrictive on single-user machines. On shared Windows systems, verify that your output folder is not accessible to other users.
