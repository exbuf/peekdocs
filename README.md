...
> Search PDFs, Word docs, spreadsheets, and 26 other file types — all at once, all offline. Use plain keywords, regex, Boolean logic, fuzzy matching, or range queries to find exactly what you need. Build compliance suites that run the same checks on a schedule and produce pass/fail audit reports. From a home user searching personal files to an auditor reviewing 500 contracts, it just works. Free, open-source, no subscription fees. Mac/Windows/Linux.
