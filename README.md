...
peekdocs es una plataforma local de búsqueda y análisis de documentos para Windows, macOS y Linux, con interfaz gráfica (GUI), línea de comandos (CLI) y API de Python. Busca en más de 100 formatos de archivo — incluyendo PDF, documentos Office (Word, Excel, PowerPoint), archivos de correo electrónico, archivos ZIP/7z, código fuente y documentos escaneados mediante OCR — usando búsquedas por palabra clave, booleanas, aproximadas, comodín, proximidad, rango y regex avanzado. Genera informes con coincidencias resaltadas en amarillo, automatiza búsquedas recurrentes, realiza análisis por lotes y guarda perfiles de búsqueda reutilizables. Gratuito y de código abierto bajo la Licencia MIT.

Diseñado para personas que prefieren herramientas locales, transparentes y deterministas.

*Todo es accesible desde la GUI — la pantalla principal, el popup **Conjuntos de Búsqueda** (botón verde junto a Ejecutar búsqueda estándar) y **Herramientas → Programar búsqueda** — esta última genera un comando de cron / Programador de tareas de Windows para que lo pegues, en lugar de instalar la programación por ti.*
...
peekdocs est un atelier local de recherche et d'analyse de documents pour Windows, macOS et Linux, disponible avec une interface graphique (GUI), en ligne de commande (CLI) et via une API Python. Recherche dans plus de 100 formats de fichiers — y compris PDF, documents Office (Word, Excel, PowerPoint), archives e-mail, fichiers ZIP/7z, code source et documents numérisés via OCR — avec des recherches par mot-clé, booléennes, approximatives, joker, proximité, plage et regex avancée. Génère des rapports avec correspondances surlignées en jaune, automatise les recherches récurrentes, effectue des analyses par lots et enregistre des profils de recherche réutilisables. Gratuit et open source sous licence MIT.

Conçu pour les personnes qui préfèrent les outils locaux, transparents et déterministes.

*Tout est accessible depuis l'interface graphique — la page principale, le popup **Suites de recherche** (bouton vert à côté de Exécuter la recherche standard) et **Outils → Planifier la recherche** — ce dernier génère une commande cron / Planificateur de tâches Windows à coller, plutôt que d'installer la planification à votre place.*
...
peekdocs ist eine lokale Dokumenten-Such- und Analyseplattform für Windows, macOS und Linux, verfügbar als grafische Benutzeroberfläche (GUI), Kommandozeile (CLI) und Python-API. Durchsucht über 100 Dateiformate — einschließlich PDF, Office-Dokumente (Word, Excel, PowerPoint), E-Mail-Archive, ZIP/7z-Dateien, Quellcode und gescannte Dokumente per OCR — mit Stichwort-, booleschen, unscharfen, Platzhalter-, Nähe-, Bereichs- und erweiterten Regex-Suchen. Erstellt Berichte mit gelb hervorgehobenen Treffern, automatisiert wiederkehrende Suchen, führt Stapelanalysen durch und speichert wiederverwendbare Suchprofile. Kostenlos und Open Source unter MIT-Lizenz.

Entwickelt für Menschen, die lokale, transparente und deterministische Werkzeuge bevorzugen.

*Alles ist über die GUI zugänglich — die Hauptseite, das **Such-Suiten**-Popup (grüner Button neben Standardsuche ausführen) und **Werkzeuge → Suche planen** — letzteres erzeugt einen cron- / Aufgabenplaner-Befehl zum Einfügen, statt den Zeitplan für Sie zu installieren.*
...
peekdocs は Windows、macOS、Linux 向けのローカルドキュメント検索・分析ワークベンチで、GUI、コマンドライン (CLI)、Python API として利用できます。100 以上のファイル形式 (PDF、Office ドキュメント (Word、Excel、PowerPoint)、メールアーカイブ、ZIP/7z ファイル、ソースコード、OCR によるスキャンドキュメント) を、キーワード、ブール、あいまい、ワイルドカード、近接、範囲、高度な正規表現検索で検索します。一致箇所が黄色でハイライトされたレポートを生成し、定期的な検索を自動化し、バッチ分析を実行し、再利用可能な検索プロファイルを保存できます。MIT ライセンスの下で無料・オープンソース。

ローカル、透明性のある、決定論的なツールを好む人のために構築されています。

*すべての機能は GUI からアクセスできます — メインページ、**検索スイート** ポップアップ (標準検索の実行ボタン横の緑色のボタン)、および **ツール → 検索のスケジュール** — 最後のものはスケジュールをインストールするのではなく、貼り付け用の cron / タスクスケジューラ コマンドを生成します。*
...
peekdocs 是一款适用于 Windows、macOS 和 Linux 的本地文档搜索与分析工作台，提供 GUI、命令行 (CLI) 和 Python API。可搜索超过 100 种文件格式 — 包括 PDF、Office 文档 (Word、Excel、PowerPoint)、电子邮件存档、ZIP/7z 文件、源代码以及通过 OCR 处理的扫描文档 — 使用关键词、布尔、模糊、通配符、邻近、范围和高级正则表达式搜索。生成匹配项以黄色高亮显示的报告、自动化重复搜索、执行批量分析，并保存可重用的搜索配置文件。基于 MIT 许可证免费开源。

为偏好本地、透明、确定性工具的人士而构建。

*所有功能均可通过 GUI 访问 — 主页面、**搜索套件** 弹窗（运行标准搜索按钮旁的绿色按钮），以及 **工具 → 计划搜索** — 最后一项会生成一条 cron / 任务计划程序命令供您粘贴，而不是为您安装计划任务。*
...
*Tudo é acessível pela GUI — a página principal, o popup **Conjuntos de Pesquisa** (botão verde ao lado de Executar pesquisa padrão) e **Ferramentas → Agendar Pesquisa** — esta última gera um comando cron / Agendador de Tarefas do Windows para você colar, em vez de instalar o agendamento por você.*
...
**Foundation — read, query, output, trust, compatibility:**
...
**Search depth — how peekdocs goes beyond grep:**
...
- **Regex Search** — edit up to 10 named regex patterns at a time in the popup workbench; collections themselves are unbounded in size (run from disk via the CLI or the in-popup **Run Multiple Collections** picker). Switch between collections for different tasks (e.g., "code patterns", "log analysis", "invoice extraction"), or fan out across several at once. Also runnable from Python via `run_regex_collection()`.
- **Folder watcher (`--watch`)** — long-running CLI mode: watch a folder, run a named regex collection on every file create / modify, emit one NDJSON record per match to stdout. Pipe straight into a log file, `jq`, or any log shipper that reads JSON Lines. Same 100+ file types peekdocs reads natively — no separate Word / PDF / Excel adapters needed in the pipeline.
...
- **Repeatable workflows** — Saved Searches, **Search Suites**, Regex Collections, Schedule Search, Search History, and Diff Snapshots compose into a workflow system: name a search and reload it later; group related searches into a Suite and run them all with one click (or fan out across several via the in-popup **Run Multiple Search Suites** picker); reuse pattern sets via Regex Collections; schedule any suite to run on a cadence via cron / Task Scheduler; audit every run via Search History; compare today's run against last week's via Diff Snapshots. `run_suite()` and `run_regex_collection()` are exposed via the Python API for programmatic use.

**Integration — driving it from code and pipelines:**

- **Three interfaces, one engine** — same search engine and same behavior across the GUI, CLI, and Python API. Schemas (Search Suites, Regex Collections, saved searches, report formats) are shared regardless of which surface you use, so a search you build by hand in the GUI today can be driven from a Python script or a cron job tomorrow with identical results.
- **Composes with standard Unix tools** — the CLI is built to participate in shell pipelines. `--stdout` writes JSON to stdout (banners suppressed); `--watch --regex-collection NAME` streams NDJSON one record per match; `--diff … --json` and `--runs --json` emit machine-readable output; status and warnings go to stderr so the stdout stream stays a clean pipe. Pipe directly into `jq`, `awk`, `sort`, log shippers (Splunk / Datadog / Filebeat), or any tool that reads JSON / JSON Lines — no preprocessing needed. Exit codes follow shell convention: `0` = success / matches found, `1` = no matches, `2` = error; `--diff` returns `1` when changes are detected so the result composes with `&&` / `||`.
- **Scriptable & deterministic** — Diff Snapshots, Schedule Search command generation, and a stable CLI surface for cron jobs, CI pipelines, log shippers, and shell pipelines. Same inputs produce the same matches every time within a release. The CHANGELOG documents behavior changes between releases; pin to a specific version if you need output stability across upgrades.

**Polish & affordances — quality-of-life and opt-in features:**

- **Polished UI** — dark / light / system appearance modes (persists across sessions), adjustable text size, tooltips on virtually every control with examples and context, contextual **?** help popups throughout the app (Getting Started, Search Wizard, Regex Wizard, Regex Tester, Recent Searches, Diff Snapshots, Advanced Search Options, and more), bash-style **↑** / **↓** recall of recent searches in the search bar, a live elapsed-time counter during long scans, status-line hints (e.g., "Tip: build an index for faster searches" after a 100+ file scan), and preferences that auto-persist to `~/.peekdocsrc` on every change — no Save button to remember.
- **OCR** — search scanned PDFs and images. Tesseract (free, open-source) must be installed separately — but once it is, peekdocs handles the rest. *Accuracy depends on source quality: clean printed pages work well; handwriting, low-resolution scans, and complex layouts may extract poorly.* **First run on a folder of scanned PDFs is much slower** (Tesseract has to extract text from every page — e.g., a 300-scanned-PDF folder takes 20–30+ seconds the first time); subsequent searches read the extracted text from the cached search index and return at normal indexed-search speed (~sub-second). Deleting `.peekdocs.db` or moving the corpus to a fresh machine forces the OCR cost to be paid again. **If the file-type count in your results looks lower than expected**, check whether OCR is on — when it's off (the default), image files (`.bmp`, `.jpg`, `.png`, `.tif`, etc.) aren't included in the searched corpus at all, and scanned PDFs return zero text. Enable OCR in Advanced Search Options (or pass `-O` on the CLI) so those formats join the count.
