...
- **AI/ML engineers** — search training logs for specific metrics, hyperparameters, or error messages across experiment runs. Find every reference to a model name, checkpoint path, or dataset version across notebooks, scripts, and configs. peekdocs reads Jupyter notebooks (`.ipynb`), which grep and most search tools can't parse. Search across READMEs, docstrings, and markdown files for outdated model names or deprecated API versions. Use PII Scan on CSV/Excel training data to find SSNs, emails, or phone numbers before publishing a dataset or model.
