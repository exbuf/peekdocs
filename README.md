...
- **General advice for checking any open-source software for telemetry:**
  ...
  - **Runtime test** (macOS/Linux): while the software is running, open a terminal and run `sudo lsof -i -n -P | grep -i python`. This shows all active network connections made by any Python process. If nothing appears, the software is making zero outbound connections — unlike the grep check (which examines source code), this checks actual behavior and catches dynamic imports, subprocess calls, and dependency-level networking.
