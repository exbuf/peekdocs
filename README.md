...
- **AI assistant integration (MCP)** — an optional, read-only [Model Context Protocol](https://modelcontextprotocol.io) server (`peekdocs-mcp`) lets an MCP-capable AI assistant search your documents through the same engine. It exposes search and listing only — no write, move, rename, or delete — and a required `--root` fences it to the folders you name. Cloud assistants (**Claude Desktop, Claude Code**) run the AI on the vendor's servers, so the matches they read are sent off your machine; pair peekdocs instead with a **free AI model that runs on your own computer** (Llama, Qwen, or Mistral, via Ollama or LM Studio) to keep everything local. See the [User Guide → MCP server](docs/USER_GUIDE.md#mcp-server-search-from-an-ai-assistant) for setup, usage examples, and the fully-local configuration.
