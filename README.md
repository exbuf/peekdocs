...
**Free · Open-Source · No Cloud · Searches 86 file types all at once · Tested on 1,000,000 files**
...
> peekdocs **document search app** searches your Word docs, PDFs, spreadsheets, emails — 86 file types, all searched at once, all offline. Yellow-highlighted results, sensitive-data (PII) scanning, text-from-images (OCR) for scanned documents, and search modes from simple keywords to advanced patterns (regex).
>
...
>
> <p align="center">Point-and-click GUI &nbsp;&nbsp;•&nbsp;&nbsp; Terminal CLI &nbsp;&nbsp;•&nbsp;&nbsp; Python API</p>
...
- **10 search modes** — plain keywords, AND/OR, Boolean expressions (`(budget OR revenue) AND NOT draft`), regex, wildcards, fuzzy matching (typo-tolerant), whole-word, proximity (terms within N words of each other), inverse search (find files that DON'T contain a term), and range queries (filter by dollar amounts, dates, percentages, ages, file sizes).
...
- **Home users** — every programmer and engineer is also a home user with tax returns, medical records, insurance policies, and a laptop they might sell someday. Type your keyword(s), click Run Search, done. No setup, no configuration, no manual. Click **PII Scan** to check your files for Social Security numbers, credit cards, and other sensitive data — one click, no setup
