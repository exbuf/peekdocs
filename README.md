...
- **OCR** — search scanned PDFs and images. Tesseract (free, open-source) must be installed separately — but once it is, peekdocs handles the rest. *Accuracy depends on source quality: clean printed pages work well; handwriting, low-resolution scans, and complex layouts may extract poorly.* **If the file-type count in your results looks lower than expected**, check whether OCR is on — when it's off (the default), image files (`.bmp`, `.jpg`, `.png`, `.tif`, etc.) aren't included in the searched corpus at all, and scanned PDFs return zero text. Enable OCR in Advanced Search Options (or pass `-O` on the CLI) so those formats join the count.
