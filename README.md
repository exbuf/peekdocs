...
### <img src="https://flagcdn.com/w40/es.png" width="28" alt="ES"> Español
...
### <img src="https://flagcdn.com/w40/fr.png" width="28" alt="FR"> Français
...
### <img src="https://flagcdn.com/w40/de.png" width="28" alt="DE"> Deutsch
...
### <img src="https://flagcdn.com/w40/jp.png" width="28" alt="JP"> 日本語
...
### <img src="https://flagcdn.com/w40/cn.png" width="28" alt="CN"> 简体中文
...
### <img src="https://flagcdn.com/w40/br.png" width="28" alt="BR"> Português brasileiro
...
<!--
  ...
  2. Convert to a looping GIF with ffmpeg:
       ffmpeg -i hero2.mp4 -vf "fps=10,scale=720:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5:diff_mode=rectangle" -loop 0 hero2.gif
  3. Replace docs/images/hero2.gif. Current file is ~11 MB at
     720p/10fps. The <img width="720"> attribute below
...
*Prefer to pause, rewind, or scrub to a specific moment? All five clips in this README are also available as pausable MP4s on the maintainer's [personal site](https://robertdschoening.com/peekdocs).*

<img src="docs/images/hero2.gif" width="720" alt="peekdocs Standard Search mid-run — same budget search the caption describes, looping">

*A Standard Search: peekdocs searches for `budget` across a 10,412-file folder and reports back in 2.98 seconds\*, with matches highlighted in yellow in the preview pane. The clip then opens the **File Types** and **Categories** charts to show the breadth of what was searched in that single pass — PDFs, Word and Excel docs, slides, emails, e-books, OCR'd images, archives, source code, and plain text. \* MacBook M4 Pro*
...
peekdocs ships an optional [Model Context Protocol](https://modelcontextprotocol.io) server so an MCP-capable AI assistant can search your local documents — over the same engine the CLI and GUI use. You ask about your files in plain, everyday language — for example, *"find the contract that mentions the roof warranty"* — and the assistant does the searching for you, so you never have to learn any search commands. It works with any MCP host: a **cloud** assistant (Claude Desktop, Claude Code) is easiest, or pair it with a **free AI model that runs on your own computer** (Llama, Qwen, or Mistral, via [Ollama](https://ollama.com) or [LM Studio](https://lmstudio.ai)) to keep everything on your machine — see the privacy note below.
...
- **IT consultant** — Search a folder of client documents for a set of terms. Or carry the standalone binary on a USB stick and run against a client's drive without installing anything — `--output-dir` back to the USB, `--no-index` to leave zero artifacts on the client machine. See [Portable use](docs/USER_GUIDE.md#portable-use--running-peekdocs-from-a-usb-stick) for the full workflow — the section now also covers personal / evaluation use for locked-down machines, air-gapped environments, and "try before you install."
...
- **Anyone who'd rather *ask* than build a search (AI assistant — optional)** — Connect peekdocs to an MCP-capable AI assistant and search by asking in plain language; the assistant runs the searches and answers with file-and-line citations, grounded in your real documents rather than guessed. For example — a **small-business owner:** *"which contract mentions the roof warranty?"*; a **developer:** *"which files still call the deprecated `authV1` function?"*; an **auditor:** *"run my Evidentiary Patterns collection over this folder and group the hits by document"*; a **researcher:** *"which PDFs cite the Smith 2019 method?"*; an **AI/ML engineer:** *"which experiment logs mention checkpoint `run_2024_ckpt_9`?"*; an **IT consultant:** *"search this client folder for the old API-key format"*; a **data researcher:** *"which of these CSVs contain account number 88213?"*; an **engineer:** *"which datasheets mention a tolerance of ±0.5%?"*; a **tech writer:** *"find every doc still using the old product name"*; an **office worker:** *"which files mention Acme and a dollar amount?"* Works with a cloud assistant (Claude Desktop / Code) or a free local model — see [MCP server](#mcp-server-optional--search-from-an-ai-assistant).
...
- **You always know *why* a document was selected.** Embedding-based retrieval ranks chunks by an *approximate* similarity score; peekdocs matches exactly — same query, same results, every hit auditable.
...
- **AI assistant integration (MCP)** — an optional, read-only [Model Context Protocol](https://modelcontextprotocol.io) server (`peekdocs-mcp`) lets an MCP-capable AI assistant search your documents through the same engine. It exposes search and listing only — no write, move, rename, or delete — and a required `--root` fences it to the folders you name. Pair peekdocs with a **free AI model that runs on your own computer** (Llama, Qwen, or Mistral, via Ollama or LM Studio) to keep everything local. Cloud assistants (**Claude Desktop, Claude Code**) run the AI on the vendor's servers, so the matches they read are sent off your machine. See the [User Guide → MCP server](docs/USER_GUIDE.md#mcp-server-search-from-an-ai-assistant) for setup, usage examples, and the fully-local configuration.
...
- **Search index** — optional SQLite FTS5 index for faster repeated searches; also powers optional **relevance ranking** (`--rank`) — a deterministic BM25 sort that floats the most on-point matches first, without changing *which* matches are found
...
**What makes peekdocs distinctive is the combination.** Local-only operation, privacy-first design, grep-like search power, OCR, regex workflows, highlighted reporting, and automation — across 100+ heterogeneous document formats — rarely come together in one tool. Each capability has good options elsewhere on its own; peekdocs delivers all of them at once, on your own machine, through a single engine shared by the GUI, CLI, and Python API. The optional MCP server extends that same engine to AI assistants — and paired with a local model, it carries the privacy-first promise all the way to conversational search that never leaves your computer.
...
**And when you add AI, it stays yours.** As AI assistants spread, most run in the cloud — so your documents, or the snippets they read, leave your machine. peekdocs offers the same *ask-in-plain-language* experience through its optional [MCP server](#mcp-server-optional--search-from-an-ai-assistant), but it drives the *same local, deterministic engine* — and paired with a model running on your own computer, the whole loop (question, snippets, answer) stays offline. That's the hard-to-copy part: **private, local, deterministic AI-era search**, not AI running on someone else's server.
...
4. **Already have image-only PDFs?** If you have a backlog of scans without a text layer, [ocrmypdf](https://github.com/ocrmypdf/OCRmyPDF) (free, open-source, runs locally) adds a text layer in place. Install with `brew install ocrmypdf` (macOS), `pipx install ocrmypdf` (Windows), or `sudo apt install ocrmypdf` (Linux), then run `ocrmypdf input.pdf input.pdf` (same path twice = convert in place). Batch a folder with `for f in *.pdf; do ocrmypdf --skip-text "$f" "$f"; done` — `--skip-text` leaves already-searchable PDFs alone, so it's safe to re-run. Once converted, peekdocs finds them with a plain text search — no `-O` flag, and none of OCR's slowdown. peekdocs itself never modifies your PDFs; ocrmypdf is a separate tool you opt into for permanent conversion.
