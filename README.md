...
| **System administrators and IT staff at small-to-mid-sized organizations** | Concrete profiles peekdocs fits well: solo IT at a doctor's office or clinic (HIPAA), a small law firm (e-discovery, privileged material), a financial advisor or CPA practice (SEC/FINRA/IRS), a school district or municipal office (FERPA, citizen data); independent IT consultants or MSP techs who visit multiple client sites; compliance officers at regulated SMBs; security consultants doing point-in-time assessments. The common thread: one person owns the problem end-to-end, has no budget for enterprise DLP (Varonis, Spirion, Purview), needs reports that hold up to an auditor, and works on a mix of Mac / Windows / Linux. See [Automation and IT Use](docs/USER_GUIDE.md#automation-and-it-use) in the User Guide for exit codes, `--stdout` JSON schemas, scheduled-scan patterns, the `--diff` / `--hash` / `--on-match` flags, and where reports and the run log live on disk | Quarterly compliance scans for sensitive data; e-discovery and forensic triage; nightly cron jobs that scan shares and notify on findings; preflight (`--dry-run`) on unfamiliar network shares; weekly trend tracking via `--diff`; portable scanning on a USB stick at client sites |
...
| **cron** | A built-in Unix/Linux/macOS scheduler that runs commands automatically on a schedule — every hour, every Monday at 8am, the first of every month, etc. On Windows the equivalent is **Task Scheduler** (or its CLI form `schtasks`). peekdocs's `--suite`, `--regex-collection`, `--timestamp`, `--stdout`, `--on-match`, and `--diff` flags are designed to compose into cron / Task Scheduler jobs for unattended compliance scans. The Tools → Schedule Search dialog generates the correct command for your OS automatically |
| **Diff** | A side-by-side comparison of two files, datasets, or scan results — the term is borrowed from the Unix `diff` command (1974). peekdocs's `--diff old.json new.json` compares two JSON snapshots and reports what's new, removed, changed, or modified — the closing piece of an IT compliance loop ("what's *new* since last week?"). Exit codes follow diff convention: 0 = no actionable change, 1 = new findings detected, 2 = error |
...
| **Hash (SHA-256)** | A short, fixed-length fingerprint (64 hexadecimal characters) computed from a file's raw bytes. Any change to the file — even a single byte — produces a completely different hash, so two files with the same SHA-256 are mathematically certain to be byte-identical. peekdocs uses SHA-256 for chain-of-custody in forensic and compliance work: add `--hash` to any `--stdout` or `-o json` run and each matched file's `sha256` is included in the JSON, letting an auditor or investigator prove later that "this is the exact file I found" even after the original has been modified, moved, or deleted. Same algorithm used by Git, Bitcoin, and most modern security tools |
