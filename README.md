...
- **Folder watcher (`--watch`)** — long-running CLI mode: watch a folder, run a named regex collection on every file create / modify, emit one NDJSON record per match to stdout. Pipe straight into a log file, `jq`, a webhook, or any SIEM / log-shipper (Filebeat, Splunk, Datadog) that reads JSON Lines. Real-time pattern detection across the same 100+ file types peekdocs reads natively — no separate Word / PDF / Excel adapters needed in your pipeline.
