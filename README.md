...
Several ways: (1) **Read the source code** — it's fully open on [GitHub](https://github.com/exbuf/peekdocs). All Python files are readable; `grep -r "http\|socket\|urllib\|requests" peekdocs/` checks for common networking libraries — if it returns nothing, no obvious network calls are present in the peekdocs source code. Note: this is a quick first-pass check, not a full security audit — it won't detect dynamic imports, subprocess-based network calls, or network activity in third-party dependencies. These same techniques (grep for networking libraries, source code review, sandbox testing, Wi-Fi-off testing) work for checking any open-source software for telemetry, not just peekdocs. For deeper assurance, read the source code directly or run peekdocs in a sandbox with network monitoring. Another quick check: turn off your Wi-Fi and verify peekdocs works normally. This confirms it doesn't require a network connection, though it won't detect software that silently retries when connectivity returns. (2) **Check dependencies** — `pip download peekdocs` downloads without installing so you can inspect the code; all 17 dependencies are well-known PyPI packages with thousands of users. (3) **Build from source** — clone the repo, read the code, `pip install -e .` — you know exactly what you're running. (4) **Scan the standalone exe** — upload it to [VirusTotal](https://www.virustotal.com), which scans with 70+ antivirus engines (note: PyInstaller executables often trigger 1-2 false positives because the bundling technique resembles malware packers — this is normal for legitimate open-source tools). (5) **Run in a sandbox** — Windows Sandbox, a VM, or a Docker container. peekdocs has no network calls, no telemetry, and no background processes.
