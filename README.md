...
|-----|--------------|---------------------|
| **Developers and technical power users** | CLI + JSON stdout, regex collections, Python API, exit codes, search index, automation | Search source trees for patterns, analyze logs, scan repos for TODO/FIXME, build automation scripts, integrate into workflows, generate machine-readable JSON reports |
| **Researchers and academics** | OCR, Boolean search, fuzzy matching, proximity search, highlighted reports, multiple output formats | Literature review across hundreds of PDFs, search years of notes, find themes across research collections, search scanned historical documents, extract references and citations |
| **Investigators and analysts** | Boolean logic, regex workflows, batch analysis, highlighted reports, export formats, inverse search | Review large archives, investigate document collections, contract review, timeline reconstruction, email archive analysis |
| **System administrators and IT staff** | Local/offline, recursive search, regex, automation, JSON output, scheduling, CLI over SSH | Search config files, review logs, inventory analysis, scan backups and document shares, identify patterns in text dumps |
| **Consultants and independent analysts** | Privacy-first, portable, no cloud, works on client data, open-source, automation-friendly | One-off document analysis, client archive review, research and report generation, technical due diligence |
| **Knowledge workers and power office users** | GUI, Search Wizard, prebuilt search types, highlighted reports, no syntax required | Find contracts, search invoices, review budgets, search years of Word/PDF files, scan scanned documents |
| **Engineers** | 100+ file types including SPICE/Verilog/VHDL/DXF/MATLAB, OCR for scanned drawings, highlighted Word reports for design reviews | Search datasheets, specs, test reports, maintenance records, standards references, vendor documents |
